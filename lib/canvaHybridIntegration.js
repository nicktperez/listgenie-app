...
class CanvaHybridIntegration {
  ...
  // Generate Canva project link (using real template links)
  generateCanvaProjectLink(projectSpec) {
    // Instead of fake project URLs, provide a direct link to Canva's real estate templates
    // Users can browse and select the perfect template for their needs
    
    const template = projectSpec.template;
    
    // Direct link to Canva's real estate flyer templates
    const canvaUrl = 'https://canva.com/templates/search/real-estate-flyer';
    
    return {
      url: canvaUrl,
      projectId: `browse_templates`,
      status: 'templates_available',
      accessType: 'browse_and_copy',
      expiresAt: null, // Templates don't expire
      instructions: 'Browse real estate flyer templates and copy the one you like'
    };
  }

  // Get real Canva template IDs for real estate flyers
  getTemplateId(style) {
    // We're now directing users to browse templates instead of specific IDs
    // This ensures the links always work
